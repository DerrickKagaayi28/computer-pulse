...
$netAdapter = Get-NetIPAddress -AddressFamily IPv4 -ErrorAction SilentlyContinue | Where-Object {$_.InterfaceAlias -notlike "*Loopback*" -and $_.IPAddress -notlike "169.254*"} | Select-Object -First 1 
...
$latency    = if ($pingResult) { "$($pingResult.ResponseTime) ms" } else { "Host Unreachable" } 
